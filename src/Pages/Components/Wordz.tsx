...
export default function Wordz() {
  const wordInterval = useRef<number>() as any
  ...
  function startWords() {
    words.current = Array.from(
      document.getElementsByClassName("word") as HTMLCollectionOf<HTMLElement>
    )
  ...
  function changeWord() {
    ...
    const nw =
      currentWord.current === words.current.length - 1
        ? wordArray.current[0]
        : wordArray.current[currentWord.current + 1]
  ...
  return (
    <div className="relative h-[100%] flex justify-center items-center">
      <div className="bannertxt text-3xl">
        <label>
          <span className="word text-center sm:text-start text-secondary">
            Dream &nbsp; on. &nbsp;{" "}
          </span>
          <span className="word text-center sm:text-start text-secondary">
            Dare &nbsp; it
